...
class AudioBeepDetector(
    ...
    private val MAG_THRESHOLD = (1e6 * WINDOW_SIZE).toFloat() //2026-03-12 added WINDOW_SIZE
                ...
                while (running)
                {
                    ...
                    while (pos + WINDOW_SIZE <= read)
                    {
                        ...
                        for (i in 0 until WINDOW_SIZE) {
                            val s = audioBuf[pos + i].toFloat() * hann[i]
                        ...
                        // Сначала проверяем порог громкости (это быстрее, чем спектр)
                        var detected = false
                        if (main > MAG_THRESHOLD) {
                        ...
                        if (detected) {
                            ...
                            if (silenceWindows > SILENCE_WINDOWS_LIMIT) {
                                val beepEndSample = currentWindowGlobalSample - (SILENCE_WINDOWS_LIMIT * STEP_SIZE)
                                val duration =
                                    (beepEndSample - beepStartSample).toDouble() / SAMPLE_RATE
//                             Log.d("MYTAG", "duration=$duration")
                                processBeepDuration(duration)
                                isBeeping = false
                                silenceWindows = 0
                            }
                        }
                    ...
                    // Увеличиваем на реально пройденное расстояние
                    totalSamplesProcessed += read
                }
            } catch (e: Exception) {
                Log.e("AudioBeepDetector", "Error in worker thread", e)
            } finally {
                ar.stop()
                ar.release()
            }
    ...
    private fun processBeepDuration(duration: Double)
    {
        when
        {
